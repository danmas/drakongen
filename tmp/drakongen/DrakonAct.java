...
public class DrakonAct {
	...
	private static String MAIN_DG_FILE = "../../../WRK/DG2J/Schemes/Main.graphml";
	Vertex in;
	...
	protected void activate_siluet(Vertex v) {
		...
		// -- ---
		DrakonUtils
				.message("--->Текущая реальность из Акт: " + CURRENT_RELEASE);
	...
	public void parseSiluet(Vertex node) {
		...
		case DI_DG_LIFT_DOWN:
			...
			try {
				...
				// -- создаем новый DrakonGen
				dg = new DrakonGen2();

				// -- устанавливаем текущую реальность
		...
		case DrakonUtils.DI_SI_BEG:
			...
			// -- ---
			DrakonUtils.message("--->Текущая реальность из Начало: "
					+ CURRENT_RELEASE);
			// -- производим разбор силуэта
			dg.parseSiluet(node);
	...
	public static void main(String[] args) {
		...
		MAIN_DG_FILE = "C:/ERV/WRK/drakongen/work/Schemes/test_REF.graphml";
